...
class BybitAPI:
    ...
    def get_ticker(self, symbol: str) -> Dict:
        """Get ticker for a single symbol — tries spot then linear"""
        for category in ('spot', 'linear'):
            try:
                data = self._make_request('GET', '/v5/market/tickers', params={'category': category, 'symbol': symbol}, auth=False)
    ...
    def get_lot_size_filter(self, symbol: str, category: str = 'spot') -> dict:
        """Get lot size filter for a symbol (cached). Returns basePrecision digits."""
        cache_key = f"{category}:{symbol}"
        if cache_key not in self._lot_size_cache:
            try:
                info = self.get_instruments_info(category, symbol)
                items = info.get('result', {}).get('list', [])
                if items:
                    ls = items[0].get('lotSizeFilter', {})
                    self._lot_size_cache[cache_key] = ls
                    return ls
            except Exception as e:
                self.logger.warning(f"Failed to get lot size for {symbol}: {e}")
            self._lot_size_cache[cache_key] = {}
        return self._lot_size_cache[cache_key]

    def round_qty_by_lot_size(self, symbol: str, qty: str | float, category: str = 'spot') -> str:
        """Round quantity to valid step size for the symbol."""
        ls = self.get_lot_size_filter(symbol, category)
        base_precision = ls.get('basePrecision', '0.00000001')
        step = float(base_precision)
        if step <= 0:
            return str(qty)
        # Round down to nearest step (safer for sell to avoid exceeding holdings)
        qty_float = float(qty)
        rounded = int(qty_float / step) * step
        # Determine number of decimal places from step
        step_str = base_precision.rstrip('0')
        if '.' in step_str:
    ...
    def set_trade_stop(self, category: str, symbol: str, side: str,
                       stop_loss: Optional[str] = None,
                       take_profit: Optional[str] = None,
                       sl_trigger_price: Optional[str] = None,
                       tp_trigger_price: Optional[str] = None) -> Dict:
        """Set or update SL/TP for linear positions via v5 position/trade-stop endpoint"""
        json_data: Dict[str, Any] = {'category': category, 'symbol': symbol, 'side': side}
        if stop_loss is not None:
            json_data['stopLoss'] = stop_loss
        if take_profit is not None:
            json_data['takeProfit'] = take_profit
        if sl_trigger_price is not None:
            json_data['slTriggerPrice'] = sl_trigger_price
        if tp_trigger_price is not None:
            json_data['tpTriggerPrice'] = tp_trigger_price
        return self._make_request('POST', '/v5/position/trading-stop', json_data=json_data)

    def set_stop_loss(self, category: str, symbol: str, side: str,
                      stop_loss: str, sl_trigger_price: str) -> Dict:
        """Set stop loss only"""
        return self.set_trade_stop(category, symbol, side,
                                   stop_loss=stop_loss, sl_trigger_price=sl_trigger_price)

    def set_take_profit(self, category: str, symbol: str, side: str,
                        take_profit: str, tp_trigger_price: str) -> Dict:
        """Set take profit only"""
        return self.set_trade_stop(category, symbol, side,
                                   take_profit=take_profit, tp_trigger_price=tp_trigger_price)
