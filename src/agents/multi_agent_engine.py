...
class FundamentalAnalysisAgent:
    ...
    def analyze(self, df: pd.DataFrame, symbol: str) -> Dict[str, Any]:
        ...
        vol_historical = (
            float(volume.iloc[-100:-20].mean()) if len(volume) >= 100 else vol_current
        )
        ...
        momentum = (
            float((close.iloc[-1] / close.iloc[-20] - 1) * 100)
            if len(close) >= 20
            else 0
        )
        if momentum > 5:
    ...
    def analyze(self, df: pd.DataFrame, symbol: str) -> Dict[str, Any]:
        ...
        for i in range(-1, -n, -1):
            ...
            if i < -1 and n >= 2:
                ...
                if prev_body > 0:
                    if (
                        close.iloc[i] > open_prices.iloc[i]
                        and close.iloc[i + 1] < open_prices.iloc[i + 1]
                    ):
                        if (
                            close.iloc[i] > open_prices.iloc[i + 1]
                            and open_prices.iloc[i] < close.iloc[i + 1]
                        ):
                            bullish_patterns += 2
        ...
        reasoning = (
            f"patterns={', '.join(pattern_names[-5:]) or 'none'}, score={score:.2f}"
        )
...
class NewsAnalysisAgent:
    ...
    def analyze(
        self, symbol: str, sentiment_data: Dict = None, df: pd.DataFrame = None
    ) -> Dict[str, Any]:
...
class MarketSentimentAgent:
    ...
    def analyze(self, df: pd.DataFrame, symbol: str = None) -> Dict[str, Any]:
        ...
        reasoning = (
            f"vol={volatility:.4f}, vol_ratio={vol_ratio:.2f}x, mom_5={momentum_5:.2%}"
        )
        ...
        try:
            ...
            return {
                ...
                "details": {
                    ...
                    "nearest_round_dist_pct": nearest_round.get(
                        "nearest_round_dist_pct", 0
                    ),
...
class DecisionAggregator:
    ...
    def aggregate(
        self, results: Dict[str, Dict[str, Any]], min_confidence: float = 0.35
    ) -> Dict[str, Any]:
        """
        ...
        bullish_score = (
            sum(results[b].get("score", 0.5) * self.WEIGHTS.get(b, 0) for b in results)
            / total_weight
        )
...
class MultiAgentDecisionEngine:
    ...
    def __init__(self, config: Config, logger: logging.Logger):
        ...
        self.min_confidence = config.agents.get("trading_decision", {}).get(
            "min_confidence", 0.35
        )

    def analyze(
        self, df: pd.DataFrame, symbol: str, sentiment_data: Dict = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            results["market_sentiment"] = self.blocks["market_sentiment"].analyze(
                df, symbol
            )
        except Exception as e:
