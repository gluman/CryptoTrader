...
# Sync positions from exchange before starting
print("\n[0/4] Syncing positions from exchanges...")
sync_result = executor.sync_positions_from_exchange('bybit')
print(f"  Bybit: created={sync_result.get('created', 0)}, updated={sync_result.get('updated', 0)}, closed={sync_result.get('closed', 0)}, synced={sync_result.get('synced', 0)}")
if sync_result.get('created') + sync_result.get('closed') + sync_result.get('updated') > 0:
    logger.info(f"Position sync results: {sync_result}")
