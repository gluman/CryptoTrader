...
# [06.08.2026] Хост был захардкожен на 192.168.0.149 — это СТАРАЯ БД. Боевая
# система с июля работает на локальной (POSTGRES_HOST из .env), поэтому монитор
# читал чужие данные: показывал позицию ADAUSDT от v7 при трёх реальных позициях
# v9 и "нет сигналов" при исполненных сигналах. Берём тот же источник, что и runner.
DB = dict(
...
# [06.08.2026] Было "clone5_v7_trailing_only" — v7 не торгует с июля, из-за чего
# все секции по стратегиям приходили пустыми. Берём список из боевого runner.
try:
    from cryptotrader_strategies.clone5_multi_runner import STRATEGIES as _RUNNER_STRATS
    STRATEGIES = [c["name"] for c in _RUNNER_STRATS]
except Exception:
    STRATEGIES = ["clone5_v9_ctl"]
...
def format_report(state, balance_total, balance_free, breakdown=None) -> str:
    ...
    if state["compound"]:
        ...
        lines.append(f"   Схема: {c.get('current_max_positions','?')} × "
                     f"${c.get('current_pos_usdt', 0):.2f}  ({c.get('last_update', '?')[:16]})")
    lines.append("")
    ...
    # [06.08.2026] Пустую LLM-секцию не печатаем: у v9 CTL llm_mode=off.
