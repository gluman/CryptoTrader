#!/usr/bin/env python3
"""
Clone5 v8 — АРХИВ. НЕ ИСПОЛЬЗОВАТЬ ДЛЯ ТОРГОВЛИ.

╔══════════════════════════════════════════════════════════════════════════════╗
║  ⛔ СТАТУС: АРХИВНАЯ СТРАТЕГИЯ (помечена 08.08.2026 по решению Босса)         ║
║                                                                              ║
║  ⚠️  ФАЙЛ НЕ УДАЛЯТЬ: от Clone5V8Strategy наследуется боевая Clone5V9Strategy ║
║      (clone_5_v9.py). Наследование формальное — v9 переопределяет decide()    ║
║      целиком и НЕ вызывает ни super().decide(), ни _detect_choch_*, ни        ║
║      _calculate_score, ни get_params этого класса. Через цепочку              ║
║      v8 → v7 → v6 берётся только инфраструктура (детекторы свингов, _hold,    ║
║      set_higher_tf_trends). Удаление файла сломает боевую торговлю.           ║
║                                                                              ║
║  В БОЮ БЫЛА: 03.08–04.08.2026, заменена на v9 CTL.                           ║
╚══════════════════════════════════════════════════════════════════════════════╝

ПОЧЕМУ АРХИВ — замеры на 26 боевых парах × 90 дней, комиссия 0.11% RT
(скрипты: scratchpad сессии 271ab5d3, v8_viability.py / v8_sides_grid.py):

  1. В БОЮ НЕ ТОРГУЕТ ВОВСЕ: 0 сделок за 90 дней. _detect_choch_bullish/bearish
     ищут подтверждение в барах sweep_idx+CHOCH_MIN_BARS .. sweep_idx+CHOCH_LOOKBACK,
     а decide() вызывает их с sweep_idx = len(df)-1 → range всегда пустой →
     "choch_not_confirmed" на каждом сигнале (проверено: 812 сигналов, 0 подтверждений).

  2. С ПОЧИНЕННЫМ CHOCH на боевом профиле (SL 1.2% / TP 3.5% / max_hold 15м)
     стратегия УБЫТОЧНА: 688 сделок, WR 43.2%, exp -0.061%/сделку, PF 0.83.
     Раздельно: LONG exp -0.119% (t=-2.56, значимо убыточна), SHORT +0.011% (t=0.20).
     Обе половины периода в минусе (-0.034 / -0.096).

  3. SCORE — главная надстройка v8 — РАБОТАЕТ НАОБОРОТ: ужесточение порога
     ухудшает результат (без фильтра -0.061%, score>=0.65 -0.077%,
     score>=0.70 -0.093%, score>=0.80 -0.133%). Отбор "по качеству" отбирает хуже
     случайного.

  4. Плюс появляется только подбором выхода: лучшая из 30 клеток сетки
     SL 3.5% / TP 2.5% / max_hold 360м даёт +0.199%, но t=1.91 для ЛУЧШЕГО из 30
     испытаний — незначимо. И не держится по половинам: SHORT переворачивается
     с +0.513% на -0.233%, LONG затухает с +0.272% до +0.086%. Тот же почерк
     переобучения, что в V26 и в OOS-подборе параметров.

  5. v9 CTL ДОМИНИРУЕТ по всем осям (та же выборка, обе long-only):
        v8 LONG с лучшим подобранным выходом: n=280,  exp +0.191%, t=1.38
        v9 CTL без всякого подбора выхода:    n=1654, exp +0.275%, t=5.43
     v9 — это, по сути, bullish-половина v8, ужесточённая параметрами и
     обязательным фильтром часового тренда вниз.

⚠️  ЦИФРЫ В БЛОКЕ "РЕЗУЛЬТАТЫ ТЕСТОВ" НИЖЕ НЕДОСТОВЕРНЫ — это look-ahead:
    они получены при sweep_idx в середине массива, где CHOCH-детектор "срабатывает"
    в 87% случаев, заглядывая в будущие бары. Оставлены как есть только для истории.
...
РЕЗУЛЬТАТЫ ТЕСТОВ (30д train, 7д test) — ⚠️ НЕДОСТОВЕРНЫ, look-ahead, см. шапку:
...
class Clone5V8Strategy(Clone5V7Strategy):
    """
    ⛔ АРХИВ (08.08.2026). Для торговли использовать Clone5V9Strategy.
       Класс сохранён как база наследования для v9 — не удалять, подробности
       и цифры замеров в шапке модуля.
    ...
    ⚠️ В текущем виде decide() всегда возвращает HOLD "choch_not_confirmed":
       детекторы ищут подтверждение в БУДУЩИХ барах относительно sweep_idx,
       который равен последнему бару. За 90 дней — 0 сделок.

    ⚠️ Строка "Результаты: WR 85.6%, exp +0.618%" получена с look-ahead.
       Честный замер: exp -0.061%/сделку, PF 0.83 (688 сделок, 90д, 26 пар).
    """
