#!/usr/bin/env python3
"""
Догрузка 5m OHLCV с Bybit API за последние 4.5 мес (с 20 января 2026).
Топ-пары с самыми длинными данными (BTC, ETH, SOL).
"""
import os
import sys
import time
...
# Топ-пары по ликвидности (лимит ~6-7 пар, иначе долго)
SYMBOLS = ["BTCUSDT", "ETHUSDT", "SOLUSDT", "XRPUSDT", "DOGEUSDT", "TONUSDT"]

# Скачиваем с 20 января 2026 = ~138 дней назад
END_TS = int(datetime(2026, 6, 6, 23, 59, 59, tzinfo=timezone.utc).timestamp() * 1000)
START_TS = int(datetime(2026, 1, 20, 0, 0, 0, tzinfo=timezone.utc).timestamp() * 1000)
TF = "5m"
EXCHANGE = "bybit"
