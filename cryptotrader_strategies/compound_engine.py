...
# [05.08.2026 Босс] Потолок опущен 30 → 5: размер позиции зафиксирован на $5.
# Почему: 04.08 баланс превысил $21, tier1 автоматически поднял позицию до $7 —
# то есть фактический риск на сделку оказался в 1.5 раза выше согласованного,
# а дневной стоп $3.38 из 15% депозита превратился в ~21% от трёх позиций.
# Пока эдж v9 не подтверждён живой статистикой, размер не растёт. Механизм tier'ов
# оставлен рабочим: поднять потолок — вернуть здесь нужное значение.
MAX_POS = 5  # per-pair exposure cap
MIN_POS = 1
