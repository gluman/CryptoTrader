...
STRATEGIES = [
    {
        ...
        "symbols": ["SUIUSDT", "NEARUSDT", "SOLUSDT", "LITUSDT",
                    ...
                    # [Fix 13.08.2026 Босс] ETHUSDT убран: минимальный лот 0.01 ETH
                    # при цене ~$1888 даёт позицию $18.88 вместо плановых $5 (×3.8).
                    # 11.08 такая позиция заняла почти весь депозит — свободно
                    # осталось $3.49, и 9 из 12 сигналов за день были отброшены;
                    # риск на сделке при этом был втрое выше расчётного.
                    # Вернуть можно, если депозит вырастет так, что $19 станут
                    # приемлемым размером позиции.
                    "EULUSDT", "REUSDT", "GIGGLEUSDT"],
